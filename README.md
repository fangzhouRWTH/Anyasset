...
**Status: tool v0.1.0, schema 1. Requires Python 3.11+, Git, and Git LFS.** There are no third-party Python runtime dependencies. The initial catalog contains an original PNG checker and OBJ triangle. Existing Anygine assets have not been migrated. The PNG is generated by `scripts/generate_checker.py` and tracked with LFS. Distribution tests do not establish engine importer compatibility.
...
- One engine lock selects one source repository commit; no cross-source solver or per-asset version constraints.
