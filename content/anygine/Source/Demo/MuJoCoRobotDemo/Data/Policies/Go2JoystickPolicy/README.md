...
3. `./Scripts/AI/setup-policy-worker.sh` then Start Python worker (`--backend auto` → `onnx`).
4. Validate **without** `--template` against `Assets/Data/Policies/Go2JoystickPolicy`.

With ONNX attached, Drive uses real policy actions (no demo pose-delta overlay).  
Stages / traps: `Doc/Modules/SceneSimulation/MuJoCoSimRobotDeployGuide.md`.
